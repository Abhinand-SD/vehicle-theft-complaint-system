...
$get_gri_forwardto = "";
	...
	$get_field_19  = "";
		...
		/********************************************************************/
	/*
		$result3 = mysqli_query($conn,"SELECT * FROM tat WHERE field_1='$get_field_12' AND  field_2='$get_field_5' AND  field_3='$get_type' ");
	...
	// Generate ID 
	*/
	$result2 = mysqli_query($conn,"SELECT COUNT(site) as total_balance FROM ticket WHERE site='$get_site'");
	$Allsurvey = mysqli_fetch_array($result2);
	$get_id = $Allsurvey["total_balance"] ;
	$ticke_id = $get_field_5.'-'.($get_id + 1);
	/*
	// Forward TO Emp ID
	$result4 = mysqli_query($conn,"SELECT * FROM login WHERE name='$get_field_10'");
	$Allforward = mysqli_fetch_array($result4);
	$get_for_empid = $Allforward["field_1"];
	
	// Grievance Forward To *
	
	$result5 = mysqli_query($conn,"SELECT * FROM hirarchy 
				WHERE dept='$get_field_5' AND type='$get_field_19'");
	$Allgrie_forward = mysqli_fetch_array($result5);
	...
	$result6 = mysqli_query($conn,"SELECT * FROM site_results where site='$get_site' ");
